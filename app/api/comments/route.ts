...
//      Liste paginée des avis `approved` d'une recette (created_at desc).
//      N'expose JAMAIS `author_email_hash`. Pagination obligatoire (LIMIT/OFFSET).
//
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result = await fetchApprovedComments(slug, page, COMMENTS_PAGE_SIZE);
